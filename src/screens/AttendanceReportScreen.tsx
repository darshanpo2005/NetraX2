...
interface ExportRow {
  ...
  detail: string; // check-in time for Present, reason for Absent
}

function buildExportRows(present: AttendanceRow[], absences: AbsenceRow[]): ExportRow[] {
  const presentRows: ExportRow[] = present.map(r => ({
    name: r.workerName, employeeId: r.employeeId, date: r.date, status: 'Present', detail: r.time,
  }));
  const absentRows: ExportRow[] = absences.map(a => ({
    name: a.workerName, employeeId: a.employeeId, date: a.date, status: 'Absent', detail: a.reason,
  }));
  return [...presentRows, ...absentRows];
}

function buildCSV(rows: ExportRow[]): string {
  const header = 'Name,Employee ID,Date,Status,Time/Reason';
  const lines  = rows.map(r =>
    [`"${r.name}"`, `"${r.employeeId}"`, `"${r.date}"`, `"${r.status}"`, `"${r.detail}"`].join(',')
  );
  return [header, ...lines].join('\n');
}

function buildPdfHtml(rows: ExportRow[], rangeLabel: string): string {
  const uniqueWorkers = new Set(rows.map(r => r.employeeId || r.name)).size;
  const presentCount  = rows.filter(r => r.status === 'Present').length;
  const absentCount   = rows.filter(r => r.status === 'Absent').length;
  const tableRows = rows.map((r, i) => `
    <tr style="background:${i % 2 === 0 ? '#f8fafc' : '#ffffff'}">
      <td>${r.name}</td><td>${r.employeeId || '—'}</td>
      <td>${r.date}</td>
      <td style="color:${r.status === 'Present' ? '#059669' : '#dc2626'};font-weight:700">${r.status}</td>
      <td>${r.detail}</td>
    </tr>`).join('');
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  <div class="stats">
    <div class="stat"><div class="stat-n">${presentCount}</div><div class="stat-l">Present</div></div>
    <div class="stat"><div class="stat-n">${absentCount}</div><div class="stat-l">Absent</div></div>
    <div class="stat"><div class="stat-n">${uniqueWorkers}</div><div class="stat-l">Unique Workers</div></div>
    <div class="stat" style="flex:2"><div class="stat-n" style="font-size:18px">${rangeLabel}</div><div class="stat-l">Date Range</div></div>
  </div>
  <table>
    <thead><tr><th>Name</th><th>Employee ID</th><th>Date</th><th>Status</th><th>Time/Reason</th></tr></thead>
...
export default function AttendanceReportScreen() {
  ...
  const loadAll = useCallback(async (from: Date, to: Date) => {
    ...
    try {
      ...
      const [r, a, w, t] = await Promise.all([
        getAttendanceByDateRange(fromTs, toTs),
        getAbsenceRowsForDateRange(fromTs, toTs),
        getWeeklyStats(),
        getTodayStats(),
      ]);
      setRows(r);
  ...
  const handleCSV = async () => {
    if (!rows.length && !absenceRows.length) { Alert.alert('No Data', 'No records in selected range.'); return; }
    setBusy('csv');
    try {
      const uri = (FileSystem.documentDirectory ?? '') + `netrax_${slug}.csv`;
      await FileSystem.writeAsStringAsync(uri, buildCSV(buildExportRows(rows, absenceRows)), { encoding: FileSystem.EncodingType.UTF8 });
  ...
  const handlePDF = async () => {
    ...
    setBusy('pdf');
    try {
      const { uri: tmp } = await Print.printToFileAsync({ html: buildPdfHtml(buildExportRows(rows, absenceRows), rangeLabel) });
